...
import type { FormState } from '@/type/form'
...
const contact = (_: FormState, form: FormData) =>
	send(form).then(
		...
		>
			<Field id='name'>
				<input
					...
					required
				/>
				{state.name ? <Failed message={state.name} /> : null}
			</Field>
			<Field id='email'>
				<input
					className='px-2 py-1 border-b-1 border-b-neutral-800 focus:outline-none focus:border-b-foreground'
					...
					required
				/>
				{state.email ? <Failed message={state.email} /> : null}
			</Field>
			<Field id='message'>
				<textarea
					...
					required
				/>
				{state.message ? <Failed message={state.message} /> : null}
				<style jsx>{`#message { scrollbar-width: thin; }`}</style>
			</Field>
			<button
				type='submit'
